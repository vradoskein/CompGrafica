...
public class Floodfill {
    ...
    // swapped to iterative flood fill on the actuall app
    ...
    private static void flood4Iterative(Point p1, int cor_preenche, int cor_antiga, int[][] m) {
        ...
        while (!q.isEmpty()) {
            ...
            if ((atual.x > 0 && atual.x < widthg) && (atual.y > 0 && atual.y < heightg)
                    && (m[atual.y][atual.x] == cor_antiga)) {
                m[atual.y][atual.x] = cor_preenche; // colore o pixel

                // chamadas recursivas
                Point novo = new Point(atual.x + 1, atual.y);
                q.add(novo); // dir
                novo = new Point(atual.x - 1, atual.y);
                q.add(novo); // esq
                novo = new Point(atual.x, atual.y + 1);
                q.add(novo); // baixo (y cresce para baixo no nosso canvas)
                novo = new Point(atual.x, atual.y - 1);
                q.add(novo); // cima
            }
        }
